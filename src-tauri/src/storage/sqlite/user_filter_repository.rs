...
impl UserFilterRepository {
    ...
    pub fn set_user_filter(
        ...
        let selected_users_json =
            serde_json::to_string(&selected_users).context("Failed to serialize selected users")?;
        ...
        tracing::debug!(
            "User filter saved for view: {}, context: {}",
            view_type,
            context_key
        );
        Ok(())
    ...
    pub fn get_user_filter(
        ...
        match result {
            Ok(users) => {
                tracing::debug!(
                    "Retrieved user filter for view: {}, context: {}",
                    view_type,
                    context_key
                );
                Ok(Some(users))
            ...
            Err(rusqlite::Error::QueryReturnedNoRows) => {
                tracing::debug!(
                    "No user filter found for view: {}, context: {}",
                    view_type,
                    context_key
                );
                Ok(None)
    ...
    /// ユーザーフィルタを削除
    pub fn delete_user_filter(conn: &Connection, view_type: &str, context_key: &str) -> Result<()> {
        conn.execute(
            "DELETE FROM user_filters WHERE view_type = ? AND context_key = ?",
            rusqlite::params![view_type, context_key],
        )
        ...
        tracing::debug!(
            "User filter deleted for view: {}, context: {}",
            view_type,
            context_key
        );
        Ok(())
    ...
    /// ビュータイプ全体のフィルタを削除
    pub fn delete_all_filters_by_view(conn: &Connection, view_type: &str) -> Result<()> {
        conn.execute(
...
mod tests {
    ...
    fn test_get_user_filter() {
        ...
        UserFilterRepository::set_user_filter(&conn, "project-view", "project:1", users.clone())
            .unwrap();
